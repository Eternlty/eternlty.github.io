function welcometxmap() {
    //请求数据
    const storedData = localStorage.getItem('ipLocation');
    if (storedData) {
        const data = JSON.parse(storedData);
        // 检查数据是否过期（1天有效期）
        const now = new Date().getTime();
        if (data.timestamp && now - data.timestamp < 24 * 60 * 60 * 1000) {
            ipLoacation = data.value;
            showWelcome();
        } else {
    ...
    function getLocationFromAPI() {
        ...
        window.QQmap = function (data) {
            ...
            localStorage.setItem('ipLocation', JSON.stringify(storageData));
...
function showWelcome() {
    ...
    try {
        ...
        document.getElementById("welcome-info").innerHTML =
            `<span>热烈欢迎来自~</span><br><span><span style="color: var(--icat-card-welcome);font-weight: bold;">${pos}</span> 的喵友</span><br><span class="welcome-message">${posdesc}</span><br>您当前位置距博主约 <b><span style="color: var(--icat-card-welcome);font-weight: bold;">${dist}</span></b> 公里！<br><span>您的IP地址为：<span class="ip-address">${ip}</span></span><br>${timeChange}`;
    } catch (err) {
